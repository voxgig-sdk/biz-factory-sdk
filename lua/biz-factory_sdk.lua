...
local BizFactorySDK = {}
BizFactorySDK.__index = BizFactorySDK
...
BizFactorySDK._make_feature = _make_feature


function BizFactorySDK.new(options)
  local self = setmetatable({}, BizFactorySDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BizFactorySDK:options_map()
...
function BizFactorySDK:get_utility()
  return Utility.copy(self._utility)
end


function BizFactorySDK:get_root_ctx()
  return self._rootctx
end


function BizFactorySDK:prepare(fetchargs)
...
function BizFactorySDK:direct(fetchargs)
...
-- by passing it.
function BizFactorySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BizFactorySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BizFactorySDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BizFactorySDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BizFactorySDK.new(sdkopts)
  sdk.mode = "test"
