...
class FtCtrl extends BizFactoryControl
{
...
class FtClient
{
    ...
    private BizFactoryUtility $utility;

    public function __construct(BizFactoryUtility $utility, array $options)
    {
    ...
    public function get_utility(): BizFactoryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BizFactoryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BizFactoryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BizFactoryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BizFactoryConfig::make_feature($name);
    ...
    private static function buildUrl(BizFactorySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BizFactoryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BizFactoryResponse($fetched);
        }
    ...
    private function fail(BizFactoryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BizFactoryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
