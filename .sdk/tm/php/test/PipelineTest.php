...
{
    private static function utility(): BizFactoryUtility
    {
        return new BizFactoryUtility();
    }
    ...
    private static function ctx(array $over = []): BizFactoryContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BizFactoryContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BizFactoryOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BizFactoryError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BizFactoryOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BizFactoryMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BizFactoryOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BizFactoryMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BizFactoryOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BizFactoryMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BizFactorySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BizFactoryMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BizFactoryResponse([]),
            'result' => new BizFactoryResult([]),
        ]);
        [, $err] = BizFactoryMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BizFactorySpec([]),
            'response' => null,
            'result' => new BizFactoryResult([]),
        ]);
        [, $err] = BizFactoryMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BizFactorySpec(['step' => 's']),
            'response' => new BizFactoryResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BizFactoryResult([]),
        ]);
        [, $err] = BizFactoryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BizFactorySpec(['step' => 's']),
            'response' => new BizFactoryResponse(self::resp(200, ['v' => 1])),
            'result' => new BizFactoryResult([]),
        ]);
        [, $err] = BizFactoryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BizFactorySpec(['step' => 's']),
            'response' => new BizFactoryResponse(self::resp(200, ['v' => 2])),
            'result' => new BizFactoryResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BizFactoryMakeResponse::call($ctx);
    ...
    {
        $preset = new BizFactoryResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BizFactorySpec([]),
            'response' => new BizFactoryResponse([]),
            'result' => new BizFactoryResult([]),
        ...
        [$got, $err] = BizFactoryMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BizFactoryResult([])]);
        [, $err] = BizFactoryMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BizFactorySpec([]), 'result' => null]);
        [, $err] = BizFactoryMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BizFactorySpec(['step' => 's']),
            'result' => new BizFactoryResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BizFactoryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BizFactoryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BizFactorySpec(['step' => 's']),
            'result' => new BizFactoryResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BizFactoryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BizFactoryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BizFactoryResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BizFactorySpec([]),
            'result' => new BizFactoryResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BizFactoryMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BizFactoryMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BizFactoryError('boom', 'boom');
        $utility->fetcher = function (BizFactoryContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BizFactorySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BizFactoryMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BizFactoryContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BizFactoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BizFactoryResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BizFactoryMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BizFactoryContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BizFactorySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BizFactoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BizFactoryResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BizFactorySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BizFactoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BizFactoryMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BizFactoryMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BizFactoryResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BizFactoryDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BizFactoryResult(['ok' => false])]);
        $this->expectException(BizFactoryError::class);
        BizFactoryDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BizFactoryResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BizFactoryMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BizFactoryMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BizFactoryBaseFeature();
        $b = new BizFactoryBaseFeature();
        BizFactoryFeatureAdd::call($ctx, $a);
        BizFactoryFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BizFactoryBaseFeature
    {
        $f = new BizFactoryBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BizFactoryFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BizFactoryContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BizFactorySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BizFactoryPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BizFactoryPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BizFactoryResponse(['headers' => null]),
            'result' => new BizFactoryResult([]),
        ]);
        BizFactoryResultHeaders::call($ctx);
