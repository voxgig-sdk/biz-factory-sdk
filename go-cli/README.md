...
boru-driven command-line client **and** interactive REPL for the BizFactory
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/biz-factory-cli)
make build

# 2. See usage (words, entities, env vars)
./biz-factory-cli --help
...
BIZ_FACTORY_BASE=https://api.example.com ./biz-factory-cli list group

# 6. No arguments -> interactive REPL
./biz-factory-cli
biz-factory> list group
biz-factory> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/biz-factory-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/biz-factory-cli list group
   ```
...
Run with no arguments to open a REPL (prompt `biz-factory>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `group`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/biz-factory-cli`. |
...
(`./biz-factory-cli <expr>`) and interactively (the REPL), and expressions compose the
