// Command BizFactory-cli is a CLI + REPL for the BizFactory SDK,
...
const prompt = "biz-factory"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "group"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BIZ_FACTORY_APIKEY (api key), BIZ_FACTORY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BIZ_FACTORY_APIKEY carries the API key and
	// BIZ_FACTORY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BIZ_FACTORY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BIZ_FACTORY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBizFactorySDK(opts)
