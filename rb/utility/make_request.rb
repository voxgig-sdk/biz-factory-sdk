...
module BizFactoryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BizFactoryResponse.new({})
    ...
    if fetch_err
      ...
      response = BizFactoryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BizFactoryResponse.new(fetched)
    else
