...
require_relative 'BizFactory_types'
...
class BizFactorySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BizFactoryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BizFactoryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BizFactoryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BizFactoryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BizFactoryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BizFactoryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BizFactoryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BizFactoryError.new(
        "#{op}_allow",
        "BizFactorySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BizFactoryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BizFactoryError.new(
        "graphql_error", "BizFactorySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
