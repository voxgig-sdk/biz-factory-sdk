...
class GroupEntity
  ...
  def data_set(args)
    if args
      @_data = BizFactoryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BizFactoryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GroupListMatch, Hash, nil] match filter (any subset of
  #   Group fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Group>, Array] the matching Group items; raises BizFactoryError on failure
