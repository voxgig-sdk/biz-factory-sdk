...
class GroupDirectTest < Minitest::Test
  def test_direct_list_group
    setup = group_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-group", setup[:live] ? "live" : "unit")
    if _should_skip
...
def group_direct_setup(mockres)
  ...
  if live
    ...
    client = BizFactorySDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
