...
class TestGroupEntity:
    ...
    def test_should_stream(self):
        ...
        base = BizFactorySDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BIZ_FACTORY_TEST_GROUP_ENTID JSON to run live")
        client = setup["client"]
...
def _group_basic_setup(extra):
    ...
    client = BizFactorySDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("BIZ_FACTORY_TEST_GROUP_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BIZ_FACTORY_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = BizFactorySDK(helpers.to_map(merged_opts))

    _live = env.get("BIZ_FACTORY_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BIZ_FACTORY_TEST_EXPLAIN") == "TRUE",
        "live": _live,
