...
func groupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BIZ_FACTORY_TEST_GROUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BIZ_FACTORY_TEST_GROUP_ENTID"])
	...
	if env["BIZ_FACTORY_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewBizFactorySDK(core.ToMapAny(mergedOpts))
	}
