...
import (
	...
	omni "github.com/voxgig-sdk/biz-factory-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BizFactoryError); is {
				out := map[string]any{
					"name":    "BizFactoryError",
...
func omniCtx(arg any, client *sdk.BizFactorySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BizFactorySDK); is {
			client = live
