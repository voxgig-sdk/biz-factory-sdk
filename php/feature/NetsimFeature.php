...
class BizFactoryNetsimFeature extends BizFactoryBaseFeature
{
    ...
    private function _simulate(BizFactoryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(BizFactoryContext $ctx, array $applied): void
    {
        $client = $this->client;
