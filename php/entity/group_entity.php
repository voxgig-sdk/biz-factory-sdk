...
class GroupEntity
{
    ...
    {
        if ($args) {
            $this->_data = BizFactoryHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = BizFactoryHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws BizFactoryError on failure (item-5 convention).
     */
