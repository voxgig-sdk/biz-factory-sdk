...
//     (\`new BizFactorySDK(...)\` / \`BizFactorySDK.test(...)\` become
//     \`BizFactorySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"group":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BizFactorySDK(...)\` and any
// \`BizFactorySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BizFactorySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BizFactorySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
