...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BIZFACTORY_TEST_LIVE ||
    'TRUE' === process.env.BIZFACTORY_TEST_OVERRIDE
  ) {
  ...
  m.BIZFACTORY_TEST_EXPLAIN = process.env.BIZFACTORY_TEST_EXPLAIN || m.BIZFACTORY_TEST_EXPLAIN
