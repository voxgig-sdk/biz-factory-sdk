...
[MCP](https://modelcontextprotocol.io) server exposing the BizFactory SDK as
two agent tools — `biz-factory_list` and `biz-factory_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/biz-factory-mcp)
make build

# 2. Provide credentials via the environment
export BIZ_FACTORY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user biz-factory \
  -- /absolute/path/to/biz-factory-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./biz-factory-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// biz-factory_list: first page of records
{ "entity": "group" }
{ "entity": "group", "query": { } }

// biz-factory_load: one record by id
{ "entity": "group", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/biz-factory-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user biz-factory \
     -- "$PWD"/dist/*/biz-factory-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `biz-factory_list` and `biz-factory_load` tools now appear
   in new sessions. Ask the agent to *"list group using biz-factory"*
   and it calls `biz-factory_list` with `{"entity":"group"}`.
...
```sh
export BIZ_FACTORY_APIKEY=sk_live_xxx            # API key
export BIZ_FACTORY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `biz-factory_list` tool
...
### Call the `biz-factory_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `biz-factory_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `biz-factory_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BIZ_FACTORY_APIKEY` | API key sent with every request. |
| `BIZ_FACTORY_BASE` | Optional override of the API base URL. |
...
```sh
./biz-factory-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"biz-factory_load","arguments":{"entity":"group","query":{"id":1}}}}'
```
